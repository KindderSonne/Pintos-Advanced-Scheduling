...
/* Total bytes allocated using malloc. */
static size_t malloc_bytes_allocated = 0;

/* Lock to protect malloc statistics. */
static struct lock malloc_stats_lock;
...
malloc_init (void) 
{
  ...
  printf("malloc_init called\n");
  lock_init (&malloc_stats_lock);
}
...
malloc (size_t size) 
{
  ...
  if (d == descs + desc_cnt) 
    {
      ...
      a->desc = NULL;
      a->free_cnt = page_cnt;
      lock_acquire(&malloc_stats_lock);
      malloc_bytes_allocated += page_cnt * PGSIZE;
      printf("malloc: allocating large block, size = %zu, page_cnt = %zu, malloc_bytes_allocated = %zu\n", size, page_cnt, malloc_bytes_allocated); // Thêm dòng này
      lock_release(&malloc_stats_lock);
      return a + 1;
    }
  ...
  lock_acquire(&malloc_stats_lock);
  malloc_bytes_allocated += d->block_size;
  printf("malloc: allocating normal block, size = %zu, block_size = %zu, malloc_bytes_allocated = %zu\n", size, d->block_size, malloc_bytes_allocated); // Thêm dòng này
  lock_release(&malloc_stats_lock);
  ...
  if (p != NULL)
    {
      ...
      if (d != NULL) 
      ...
      else
        {
          ...
          freed_size = a->free_cnt * PGSIZE;
          palloc_free_multiple (a, a->free_cnt);
        }
      lock_acquire(&malloc_stats_lock);
      malloc_bytes_allocated -= freed_size;
      printf("free: freeing block, freed_size = %zu, malloc_bytes_allocated = %zu\n", freed_size, malloc_bytes_allocated); // Thêm dòng này
      lock_release(&malloc_stats_lock);
    }
}
